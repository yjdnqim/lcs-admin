...
import com.fh.service.system.user.UserLoginLogService;
	...
	public Object login()throws Exception{
		...
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			String sessionCode = (String)session.getAttribute(Const.SESSION_SECURITY_CODE);		//获取session中的验证码
			 PageData userLoginLog = new PageData();
			...
			if(null == code || "".equals(code)){
				...
				pd.put("USERNAME", USERNAME);
				PageData userDb = userService.findByUId(pd);
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){
					...
					if(pd != null){
					    ...
					    try { 
					        subject.login(token); 
					        //登录成功后记录日志：
					        userLoginLog.put("USER_ID",user.getUSER_ID());
					        userLoginLog.put("LOGIN_IP",this.getRemortIP());
					        userLoginLog.put("LOGIN_TIME",Tools.date2Str(new Date()));
					        userLoginLog.put("LOGIN_STATUS","1");
					        ...
					        userLoginLog.put("LOGIN_IP",this.getRemortIP());
					        userLoginLog.put("LOGIN_TIME",Tools.date2Str(new Date()));
					        userLoginLog.put("LOGIN_STATUS","0");
					        userLoginLogService.save(userLoginLog);
					    	errInfo = "身份验证失败！";
					    }
						...
						if (userDb != null) {
							//登录失败日志
					        userLoginLog.put("USER_ID",userDb.get("USER_ID"));
					        userLoginLog.put("LOGIN_IP",this.getRemortIP());
					        userLoginLog.put("LOGIN_TIME",Tools.date2Str(new Date()));
					        userLoginLog.put("LOGIN_STATUS","0");
					        userLoginLogService.save(userLoginLog);
