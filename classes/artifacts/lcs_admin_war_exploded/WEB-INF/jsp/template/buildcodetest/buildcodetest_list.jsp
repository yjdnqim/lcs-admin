	...
	<div class="layui-card">
		<div class="layui-card-body">
			<span class="layui-breadcrumb" lay-separator="»">
				<a href="template/list.do">模板页面</a>
				<a><cite>子页面</cite></a>
			</span>
		</div>
		...
		<div class="layui-card-body">
			...
			<table class="layui-table" id="LAY-data-manage" lay-filter="LAY-data-manage">
				<thead>
				<tr>
					<th lay-data="{field:'BUILDCODETEST_ID', type: 'checkbox'}"></th>
					<th lay-data="{field:'index'}">序号</th>
					<th lay-data="{field:'F1'}">字段一</th>
					<th lay-data="{field:'F2'}">字段二</th>
					<th lay-data="{field:'F3'}">字段3</th>
...
<script>
	...
	<c:choose>
		...
		laypage.render({
			...
			jump: function(obj, first){
				if(!first){
					...
					commonUtils.redirectTo('buildcodetest/list.do', pageParams);
				}
			}
		});
	</c:if>
		...
		form.on('submit(LAY-front-search)', function (data) {
			...
			//执行search
			commonUtils.redirectTo('buildcodetest/list.do', field);
		});
		...
		var active = {
			...
			batchdel: function () {
				...
				for(var i = 0; i < checkData.length; i++){
					if(dataIDs == ""){
						dataIDs += checkData[i].BUILDCODETEST_ID;
					}else {
						dataIDs += "," + checkData[i].BUILDCODETEST_ID;
					}
				}
				layer.confirm('确定删除吗？', function (index) {
					$.ajax({
						type: "POST",
						url: '<%=basePath%>buildcodetest/deleteAll.do?tm=' + new Date().getTime(),
			...
			add: function () {
				layer.open({
					type: 2,
					title: '添加数据',
					content: '<%=basePath%>buildcodetest/goAdd.do',
					maxmin: true,
					area: ['60%', '60%'],
					btn: ['确定', '取消'],
					yes: function (index, layero) {
						...
						iframeWindow.layui.form.on('submit(' + submitID + ')', function (data) {
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>buildcodetest/save.do?tm=' + new Date().getTime(),
								data: field,
								dataType:'json',
								cache: false,
								success: function(result){
			...
			exportExcel: function () {
				window.location.href='<%=basePath%>buildcodetest/excel.do';
			}
		};
		...
		table.on('tool(LAY-data-manage)', function (e) {
			...
			if ("edit" === e.event){
				layer.open({
					type: 2,
					title: '修改数据',
					content: '<%=basePath%>buildcodetest/goEdit.do?BUILDCODETEST_ID=' + targetData.BUILDCODETEST_ID,
					maxmin: true,
					area: ['60%', '60%'],
					btn: ['确定', '取消'],
					yes: function (index, layero) {
						...
						iframeWindow.layui.form.on('submit(' + submitID + ')', function (data) {
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>buildcodetest/edit.do?tm=' + new Date().getTime(),
								data: field,
								dataType:'json',
								cache: false,
								success: function(result){
			...
			if ("del" === e.event){
				layer.confirm('确定删除吗？', function (index) {
					$.ajax({
						type: "POST",
						url: '<%=basePath%>buildcodetest/delete.do?tm=' + new Date().getTime(),
						data: {BUILDCODETEST_ID : targetData.BUILDCODETEST_ID},
						dataType:'json',
						cache: false,
						success: function(result){
							if(result.code == 0){
